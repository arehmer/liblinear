...
namespace liblinear {
    ...
    public:
	    ...
	    static float sparse_dot(const feature_node *x1, const feature_node *x2)
	    {
		    float ret = 0;
    ...
    {
    public:
	    l2r_erm_fun(const problem *prob, const parameter *param, float *C);
	    ~l2r_erm_fun();

	    float fun(float *w);
	    float linesearch_and_update(float *w, float *d, float *f, float *g, float alpha);
    ...
    protected:
	    ...
	    const problem *prob;
	    float *wx;
	    float *tmp; // a working array
	    float wTw;
	    int regularize_bias;
    };

    l2r_erm_fun::l2r_erm_fun(const problem *prob, const parameter *param, float *C)
    {
	    int l=prob->l;
    ...
    float l2r_erm_fun::linesearch_and_update(float *w, float *s, float *f, float *g, float alpha)
    {
	    int i;
	    int l = prob->l;
	    float sTs = 0;
    ...
    {
    public:
	    l2r_lr_fun(const problem *prob, const parameter *param, float *C);
	    ~l2r_lr_fun();
    ...
    l2r_lr_fun::l2r_lr_fun(const problem *prob, const parameter *param, float *C):
    ...
    {
    public:
	    l2r_l2_svc_fun(const problem *prob, const parameter *param, float *C);
	    ~l2r_l2_svc_fun();
    ...
    l2r_l2_svc_fun::l2r_l2_svc_fun(const problem *prob, const parameter *param, float *C):
	    l2r_erm_fun(prob, param, C)
    {
	    I = new int[prob->l];
    }
    ...
    {
    public:
	    l2r_l2_svr_fun(const problem *prob, const parameter *param, float *C);
    ...
    l2r_l2_svr_fun::l2r_l2_svr_fun(const problem *prob, const parameter *param, float *C):
    ...
    float l2r_l2_svr_fun::C_times_loss(int i, float wx_i)
    {
	    float d = wx_i - prob->y[i];
	    if(d < -p)
		    return C[i]*(d+p)*(d+p);
	    else if(d > p)
		    return C[i]*(d-p)*(d-p);
    ...
    void Solver_MCSVM_CS::Solve(float *w)
    {
	    ...
	    float eps_shrink = max(10.0*eps, 1.0); // stopping tolerance for shrinking
    ...
    static int solve_l2r_l1l2_svc(problem *prob, parameter *param, float *w, float Cp, float Cn, int max_iter=300)
    {
	    int l = prob->l;
	    int w_size = prob->n;
	    float eps = param->eps;
	    ...
	    while (iter < max_iter)
	    {
		    ...
		    for (s=0; s<active_size; s++)
		    {
			    ...
			    if(fabs(PG) > 1.0e-12)
			    {
                    ...
                    float zero = 0;
				    alpha[i] = min(max(alpha[i] - G/QD[i], zero), C);
        ...
        // Commented out because it causes error
	    //delete [] QD;
	    //delete [] alpha;
	    //delete [] y;
	    //delete [] index;
    ...
    static int solve_l2r_l1l2_svr(const problem *prob, const parameter *param, float *w, int max_iter=300)
    {
	    ...
	    float Gnorm1_init = -1.0; // Gnorm1_init is initialized at the first iteration
    ...
    static int solve_l2r_lr_dual(const problem *prob, const parameter *param, float *w, float Cp, float Cn, int max_iter=300)
    {
	    int l = prob->l;
	    int w_size = prob->n;
	    float eps = param->eps;
	    ...
	    float innereps = 1e-2;
	    float innereps_min = min((float)1e-8, eps);
	    float upper_bound[3] = {Cn, 0, Cp};
	    ...
	    while (iter < max_iter)
	    {
		    ...
		    for (s=0; s<l; s++)
		    {
			    ...
			    Gmax = max(Gmax, (float)fabs(gp));

			    // Newton method on the sub-problem
			    const float eta = 0.1; // xi in the paper
			    int inner_iter = 0;
            ...
            if (newton_iter <= l / 10)
			    innereps = max(innereps_min, (float)0.1 * innereps);
    ...
    static int solve_l1r_l2_svc(const problem *prob_col, const parameter* param, float *w, float Cp, float Cn, float eps)
    {
	    int l = prob_col->l;
    ...
    static int solve_l1r_lr(const problem *prob_col, const parameter *param, float *w, float Cp, float Cn, float eps)
    {
	    int l = prob_col->l;
	    int w_size = prob_col->n;
	    int regularize_bias = param->regularize_bias;
    ...
    {
	    float a_value = (*(feature_node *)a).value;
	    float b_value = (*(feature_node *)b).value;
    ...
    static int solve_oneclass_svm(const problem *prob, const parameter *param, float *w, float *rho)
    {
	    int l = prob->l;
	    int w_size = prob->n;
	    float eps = param->eps;
	    ...
	    float negGmax;                 // max { -grad(f)_i | i in Iup }
	    float negGmin;                 // min { -grad(f)_i | i in Ilow }
	    ...
	    for(i=0; i<l; i++)
	    {
		    float G = sparse_operator::dot(w, prob->x[i]);
		    if (alpha[i] == 1)
			    lb = max(lb, G);
		    else if (alpha[i] == 0)
			    ub = min(ub, G);
    ...
    static void train_one(problem *prob, parameter *param, float *w, float Cp, float Cn)
    {
	    int solver_type = param->solver_type;
	    ...
	    float primal_solver_tol = param->eps;
	    if(is_regression)
	    {
    ...
    static float calc_start_C(const problem *prob, const parameter *param)
    {
	    ...
	    else if(param->solver_type == L2R_L2LOSS_SVR)
	    {
		    ...
		    for(i=0; i<prob->l; i++)
		    {
			    y_abs = fabs(prob->y[i]);
			    sum_y += y_abs;
			    loss += max(y_abs - param->p, (float)0.0) * max(y_abs - param->p, (float)0.0);
		    }
		    if(loss > 0)
    ...
    static float calc_max_p(const problem *prob)
    {
	    int i;
	    float max_p = 0.0;
	    for(i = 0; i < prob->l; i++)
		    max_p = max(max_p, (float) fabs(prob->y[i]));

	    return max_p;
    }

    static void find_parameter_C(const problem *prob, parameter *param_tmp, float start_C, float max_C, float *best_C, float *best_score, const int *fold_start, const int *perm, problem *subprob, int nr_fold)
    {
	    // variables for CV
	    int i;
    ...
    void cross_validation(const problem *prob, parameter *param, int nr_fold, float *target)
    {
	    int i;
	    int *fold_start;
	    int l = prob->l;
